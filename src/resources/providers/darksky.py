...
class DarkSky(ContentProvider):
	...
	def forecast(self, loc, locid):
		...
		if data is not None and "currently" in data:
			...
			self.skinproperty('Current.Condition', common.ustr(item["summary"]).capitalize())
		...
		if data is not None and "daily" in data and "data" in data['daily'] and len(data['daily']['data']) > 1:
			...
			for item in data['daily']['data']:
				if index >= 1:
					...
					self.skinproperty('Day%i.Outlook' % count, common.ustr(item["summary"]).capitalize())
					...
					self.skinproperty('Daily.%i.Outlook' % count, common.ustr(item["summary"]).capitalize())
					...
					self.skinproperty('Daily.%i.Condition' % count, common.ustr(item["summary"]).capitalize())
		...
		if data is not None and "hourly" in data and "data" in data['hourly'] and len(data['hourly']['data']) > 0:
			...
			for item in data['hourly']['data']:
				...
				self.skinproperty('Hourly.%i.Condition' % count, common.ustr(item["summary"]).capitalize())
				self.skinproperty('Hourly.%i.Temperature' % count, self._2temperature(item['temperature'], iu='c',um=True))
				self.skinproperty('Hourly.%i.Outlook' % count, common.ustr(item["summary"]).capitalize())
